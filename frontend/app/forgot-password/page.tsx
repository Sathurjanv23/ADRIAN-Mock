          ...
          <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="flex justify-center mb-4">
                  <div className="w-16 h-16 rounded-2xl bg-nova-cyan/10 border border-nova-cyan/20 flex items-center justify-center">
                    <KeyRound className="w-8 h-8 text-nova-cyan" />
                  </div>
                </div>

                <div className="text-center mb-6">
                  <h1 className="text-2xl font-bold text-nova-text">Forgot Your Password?</h1>
                  <p className="text-sm text-nova-text-dim mt-2 leading-relaxed">
                ...
                <form onSubmit={handleSubmit} noValidate className="space-y-4">
                  <div>
                    <label htmlFor="forgot-email" className="text-xs font-medium text-nova-text-dim block mb-1.5">
                      Email Address
                    </label>
                    <div className="relative">
                      <Mail className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-nova-text-muted pointer-events-none" />
                      <input
                        id="forgot-email"
                        type="email"
                        value={email}
                        ...
                        autoComplete="email"
                        className={cn(
                          'w-full bg-nova-surface border rounded-xl pl-10 pr-4 py-2.5 text-sm text-nova-text placeholder:text-nova-text-muted focus:outline-none transition-all',
                          emailError
                  ...
                  {/* Demo accounts hint */}
                  <div className="p-3 rounded-xl bg-nova-surface2 border border-nova-border text-xs text-nova-text-dim space-y-1">
                    ...
                    {[
                      ...
                      <button
                        ...
                        className="flex items-center justify-between w-full px-2 py-1.5 rounded-lg hover:bg-nova-surface hover:border-nova-border border border-transparent transition-all"
                      >
                        <span className="text-nova-text-muted">{d.role}</span>
                        <span className="text-nova-cyan font-mono">{d.email}</span>
                      </button>
                    ))}
                  </div>
                ...
                <div className="flex justify-center mt-5">
                  <Link
                    href="/login"
                    className="flex items-center gap-1.5 text-xs text-nova-text-muted hover:text-nova-text transition-colors"
                  >
            ...
            {step === 'sent' && (
              ...
              >
                ...
                <p className="text-sm text-nova-text-dim mb-1 leading-relaxed">
                  A password reset link has been sent to:
                </p>
                <p className="font-semibold text-nova-cyan mb-5">{sentEmail}</p>

                <div className="p-4 rounded-xl bg-nova-surface2 border border-nova-border text-left space-y-2 mb-6">
                  {[
                    ...
                    <div key={item.text} className="flex items-start gap-2.5 text-xs text-nova-text-dim">
                ...
                {/* Demo shortcut */}
                <div className="p-3 rounded-xl bg-nova-cyan/5 border border-nova-cyan/20 text-xs text-nova-text-dim mb-5">
                  <p className="font-semibold text-nova-cyan mb-1">💡 Demo Mode</p>
                  In a real system, an email would be sent. Since this is a simulation, you can{' '}
                  {demoToken ? (
                    <Link
                      href={`/reset-password?token=${demoToken}`}
                      className="text-nova-cyan underline hover:no-underline"
                    >
                ...
                <div className="flex flex-col gap-2">
                  <button
                    ...
                    className="w-full text-sm border border-nova-border rounded-xl py-2.5 text-nova-text-dim hover:border-nova-border2 hover:text-nova-text transition-all"
                  >
                    Try a different email
                  </button>
                  <Link
                    href="/login"
                    className="flex items-center justify-center gap-1.5 text-xs text-nova-text-muted hover:text-nova-text transition-colors py-1"
                  >
                    <ArrowLeft className="w-3.5 h-3.5" />
                    Back to Sign In
                  </Link>
            ...
            {step === 'demo-reset' && (
              ...
              >
                ...
                <div className="p-4 rounded-xl bg-nova-surface2 border border-nova-border text-left text-xs text-nova-text-dim mb-5 space-y-1">
                  <p className="font-semibold text-nova-text mb-2">Demo credentials for your account:</p>
                  <div className="flex items-center justify-between">
                    <span className="text-nova-text-muted">Email:</span>
                    <span className="font-mono text-nova-cyan">{sentEmail}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-nova-text-muted">Password:</span>
                    <span className="font-mono text-nova-cyan">Demo1234</span>
                  </div>
                </div>
