...
export default function AdminOperationsCatchAll() {
    ...
    return (
      <div className="space-y-4">
        <h2 className="text-lg font-bold text-nova-text flex items-center gap-2">
          <Users className="w-5 h-5 text-nova-cyan" /> {t('heading.user_directory')}
        </h2>
        <div className="nova-card border border-nova-border rounded-xl overflow-hidden">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-nova-border/50 text-sm text-nova-text">
              {users.length > 0 ? (
                users.map((user) => (
                  <tr key={user.id} className="hover:bg-nova-surface/10 transition-colors">
                    <td className="p-4 font-semibold">{user.name}</td>
                    <td className="p-4 text-xs text-nova-text-muted">{user.email}</td>
                    <td className="p-4 text-xs capitalize text-nova-cyan">{user.role.replace('_', ' ')}</td>
                    <td className="p-4 text-xs text-nova-text-muted">{user.district ? localize(user.district) : 'National'}</td>
                    <td className="p-4">
                    ...
                    <td className="p-4">
                        ...
                        <div className="flex items-center gap-2">
                          {user.role === 'rescue_team' && (
                            <select
                              ...
                              className="bg-nova-surface2 border border-nova-border rounded px-2 py-1 text-xs text-nova-text"
                            >
                      ...
                      ) : <span className="text-xs text-nova-text-muted">Approved</span>}
                    </td>
                  </tr>
                ))
              ) : (
                <tr>
                  <td colSpan={6} className="p-8 text-center text-xs text-nova-text-dim">
  ...
  const renderOrganizations = () => {
    ...
    return (
      <div className="space-y-4">
        <h2 className="text-lg font-bold text-nova-text flex items-center gap-2">
          <Database className="w-5 h-5 text-nova-cyan" /> {t('heading.organizations')}
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {orgs.map((org) => (
            <div key={org.code} className="nova-card border border-nova-border rounded-xl p-4 space-y-3">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-sm font-bold text-nova-text">{localize(org.name)}</p>
                  <p className="text-xs text-nova-text-muted mt-0.5">{org.sector} · {localize(org.region)}</p>
                </div>
                <span className="text-[10px] font-bold px-2 py-0.5 bg-nova-cyan/10 text-nova-cyan border border-nova-cyan/20 rounded-full">{org.code}</span>
              </div>
              <p className="text-xs text-nova-text-dim">Emergency Contacts: <strong className="text-nova-text">{org.contacts}</strong></p>
            </div>
          ))}
        </div>
      </div>
  ...
  const renderResources = () => {
    return (
      <div className="space-y-4">
        <h2 className="text-lg font-bold text-nova-text flex items-center gap-2">
          <Settings className="w-5 h-5 text-nova-cyan" /> {t('heading.reserves_config')}
        </h2>
        <div className="nova-card border border-nova-border rounded-xl p-5 space-y-4">
          <p className="text-xs text-nova-text-muted uppercase tracking-wider">Stock Alert Limits</p>
          {['Water Units threshold', 'Food Packs threshold', 'Rescue Boats threshold', 'Medical Kits threshold'].map((label, idx) => (
            <div key={idx} className="flex items-center justify-between p-3 bg-nova-surface border border-nova-border rounded-lg text-sm">
              <span className="text-nova-text-dim">{label}</span>
              <input type="number" defaultValue={[200, 150, 5, 20][idx]} className="w-20 bg-nova-surface2 border border-nova-border rounded px-2 py-1 text-center font-mono text-nova-text" />
            </div>
          ))}
      ...
      <div className="space-y-4">
        ...
        <div className="nova-card border border-nova-border rounded-xl p-5 space-y-4">
          {['Severity Threshold limit (%)', 'Confidence score boundary (%)', 'Auto dispatch triggers'].map((label, idx) => (
            <div key={idx} className="flex items-center justify-between p-3 bg-nova-surface border border-nova-border rounded-lg text-sm">
              <span className="text-nova-text-dim">{label}</span>
              <input type="text" defaultValue={['70%', '85%', 'Enabled'][idx]} className="w-24 bg-nova-surface2 border border-nova-border rounded px-2 py-1 text-center font-mono text-nova-text" />
            </div>
          ))}
  ...
  const renderMonitoring = () => {
    ...
    return (
      <div className="space-y-4">
        <h2 className="text-lg font-bold text-nova-text flex items-center gap-2">
          <Activity className="w-5 h-5 text-nova-cyan" /> {t('heading.system_monitoring')}
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {servers.map((srv) => (
            <div key={srv.name} className="nova-card border border-nova-border rounded-xl p-4 flex justify-between items-center">
              <div>
                <p className="text-sm font-bold text-nova-text">{srv.name}</p>
                <p className="text-xs text-nova-text-muted mt-0.5">Uptime: {srv.uptime} · Latency: {srv.latency}</p>
              </div>
  ...
  const renderAuditLogs = () => {
    return (
      <div className="space-y-4">
        <h2 className="text-lg font-bold text-nova-text flex items-center gap-2">
          <FileText className="w-5 h-5 text-nova-cyan" /> {t('heading.audit_logs_full')}
        </h2>
        <div className="nova-card border border-nova-border rounded-xl p-3 divide-y divide-nova-border/50">
          {auditLogs.length > 0 ? (
            auditLogs.map((log) => (
              <div key={log.id} className="p-3 flex items-start gap-3 hover:bg-nova-surface/10 transition-colors">
                <span className="text-lg mt-0.5">⚙️</span>
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start gap-2">
                    <p className="text-sm font-semibold text-nova-text">{log.action}</p>
                    <span className="text-[10px] text-nova-text-muted">{timeAgo(log.timestamp)}</span>
                  </div>
                  <p className="text-xs text-nova-text-dim mt-0.5">{log.details}</p>
                  <p className="text-[10px] text-nova-text-muted mt-1">By: {log.userName} · IP: {log.ipAddress}</p>
                </div>
              </div>
            ))
          ) : (
            <div className="text-center py-6 text-xs text-nova-text-dim">
