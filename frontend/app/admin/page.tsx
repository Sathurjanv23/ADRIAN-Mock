      ...
      <DashboardShell role="admin">
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <p className="text-sm text-nova-text-dim mt-0.5">{t('portal.admin')} · PROJECT NOVA Operations Hub</p>
          </div>
          ...
          <div className="nova-card border border-nova-border rounded-xl p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <div className="ml-auto flex items-center gap-1.5 text-xs text-nova-low">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {SYSTEM_HEALTH.map((service) => (
                <div key={service.name} className={cn(
                  ...
                  service.status === 'operational' ? 'border-nova-border bg-nova-surface/50' : 'border-orange-500/30 bg-orange-500/5'
                )}>
                  ...
                  <div className="flex items-center justify-between text-[10px] text-nova-text-muted">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="nova-card border border-nova-border rounded-xl p-4">
              <div className="flex items-center justify-between mb-4">
                ...
                <Link href="/admin/users" className="text-xs text-nova-cyan hover:underline">{t('btn.manage')} →</Link>
              </div>
            ...
            <div className="nova-card border border-nova-border rounded-xl p-4">
              <div className="flex items-center justify-between mb-4">
                ...
                <Link href="/admin/ai-config" className="text-xs text-nova-cyan hover:underline">{t('btn.edit')} →</Link>
              </div>
              <div className="space-y-2">
                {AI_CONFIG.map((cfg) => (
                  <div key={cfg.name} className="flex items-center justify-between p-2 rounded-lg bg-nova-surface/50 border border-nova-border/50">
                    <span className="text-xs text-nova-text-dim">{cfg.name}</span>
                    <div className="flex items-center gap-1.5">
                      <span className="text-xs font-semibold text-nova-cyan font-mono">{cfg.value}</span>
                      {cfg.editable && <Settings className="w-3 h-3 text-nova-text-muted" />}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="nova-card border border-nova-border rounded-xl p-5">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <FileText className="w-4 h-4 text-nova-cyan" />
                <h2 className="text-sm font-bold text-nova-text">{t('heading.recent_audit_logs')}</h2>
              </div>
              <Link href="/admin/audit-logs" className="text-xs text-nova-cyan hover:underline">{t('heading.view_all')} →</Link>
            </div>
            <div className="space-y-2">
              {auditLogs.length > 0 ? (
                auditLogs.slice(0, 5).map((log) => (
                  <div key={log.id} className="flex items-start gap-3 p-3 rounded-xl bg-nova-surface/50 border border-nova-border/50">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="text-[10px] text-nova-text-muted flex-shrink-0">{timeAgo(log.timestamp)}</span>
                      </div>
                      <p className="text-xs text-nova-text-dim mt-0.5">{log.details}</p>
                      <p className="text-[10px] text-nova-text-muted mt-0.5">by {log.userName} · {log.ipAddress}</p>
                    </div>
                  </div>
                ))
              ) : (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { href: '/admin/users', icon: <Users className="w-5 h-5" />, label: t('nav.users'), color: 'text-nova-cyan border-nova-cyan/20 bg-nova-cyan/5' },
